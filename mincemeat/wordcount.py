...
def mapfn(key, value):
    for line in value.splitlines():
        ...
        #for word in lines:
        title = lines[n-1]
        #may have to loop with multiple authors for more key mapping
        author = lines[1]
        for word in title.split():
            import stopwords
            word = word.replace("-"," ")
            if(word.lower() in stopwords.allStopWords):
                continue
            for authors in author.split("::"):
                yield authors, word.lower()

def reducefn(key, value):
    from collections import Counter
    return key, Counter(value).most_common(10)
